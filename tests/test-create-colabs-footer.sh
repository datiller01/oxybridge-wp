#!/bin/bash
#
# Create Colabs Footer Template via OxyBridge API
#
# This script creates the Colabs footer template using the REST API.
# It requires authentication with WordPress application password.
#
# Usage: ./test-create-colabs-footer.sh <site_url> <username> <app_password>
#
# Example:
#   ./test-create-colabs-footer.sh https://pngna.local admin "xxxx xxxx xxxx xxxx xxxx xxxx"
#
...
check_prereqs() {
    echo "============================================="
    echo "Create Colabs Footer Template"
    echo "============================================="
    echo ""
    ...
    FIXTURE_FILE="${FIXTURES_DIR}/colabs-footer-tree.json"
    if [ ! -f "$FIXTURE_FILE" ]; then
        log_error "Fixture file not found: ${FIXTURE_FILE}"
        exit 1
    fi
    log_success "Fixture file found: colabs-footer-tree.json"

    # Count elements in fixture
    if $HAS_JQ; then
        # Count all elements in the tree recursively
        ELEMENT_COUNT=$(cat "$FIXTURE_FILE" | jq '[.. | objects | select(.id != null and .id != 1)] | length')
        log_info "Fixture contains ${ELEMENT_COUNT} elements"
    fi
...
create_template() {
    echo ""
    echo "=== Creating Colabs Footer Template ==="
    echo ""
    ...
    if $HAS_JQ; then
        ...
        HAS_ROOT=$(echo "$TREE_JSON" | jq -r 'if .root then "yes" else "no" end')
        HAS_STATUS=$(echo "$TREE_JSON" | jq -r 'if .status == "exported" then "yes" else "no" end')

        if [ "$HAS_ROOT" != "yes" ]; then
            log_error "Tree missing root element"
            exit 1
        fi
        if [ "$HAS_STATUS" != "yes" ]; then
            log_error "Tree missing status: 'exported'"
            exit 1
        fi
        log_info "Tree structure validated"
    fi
...
verify_template() {
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        if $HAS_JQ; then
            ...
            CHILD_COUNT=$(echo "$RESPONSE" | jq -r '.tree.root.children | length')

            log_info "Has tree: ${HAS_TREE}"
            log_info "Has children: ${HAS_CHILDREN}"
            log_info "Root children count: ${CHILD_COUNT}"
        fi
    else
...
print_summary() {
    ...
    if [ -n "$CREATED_TEMPLATE_ID" ]; then
        log_success "Colabs Footer template created successfully!"
        echo ""
        ...
        echo "  2. Verify the footer structure in the element tree"
        echo "  3. Check for any console errors in browser DevTools"
        echo "  4. Confirm all 43 elements are visible and styled correctly"
    else
